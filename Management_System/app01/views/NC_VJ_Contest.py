...
from app01.catch_data.VJudge_Contest import VJudgeContest
...
global sorted_team_summary_list
global nid_x
...
def contest_list(request):
    ...
    # 根据搜索条件去数据库中获取
    query_set = models.NC_VJ_Contest.objects.filter(**data_dict).order_by('-num')
    ...
    return render(request, 'NC_VJ_contest_list.html', context)


@csrf_exempt
def VJ_contest_add(request):
    """ 添加VJudge比赛 """
    # 获取输入的比赛编号
    number = request.POST.get('number')
    # print(f"xxxxx {number}")
    exists = models.NC_VJ_Contest.objects.filter(num=number).exists()
    if exists:
    ...
    # try:
    contest_object = VJudgeContest(number)
...
def contest_delete(request):
    ...
    exists = models.NC_VJ_Contest.objects.filter(id=uid).exists()
    if not exists:
        return JsonResponse({"status": False, 'error': "删除失败，数据不存在。"})
    models.NC_VJ_Contest.objects.filter(id=uid).delete()
    return JsonResponse({"status": True})
...
def select_contest_detail_list(request):
    if request.method == 'POST':
        ...
        # 处理 GET 请求，获取之前处理的数据
        # 注意：这里需要替换成你实际的查询逻辑
        team_summary = models.NC_VJ_ContestData.objects.filter(nC_VJ_Contest_id__in=contest_ids).values(
            'name').annotate(
            total_solved=Sum('solved'),
            total_penalty=Sum('time')
        )
        ...
        return render(request, 'NC_VJ_select_contest_detail_list.html',
                      {'team_summary': sorted_team_summary_list})
...
def contest_detail_list(request, nid):
    ...
    nid_x = nid
    # 获取比赛的名称
    contest = models.NC_VJ_Contest.objects.filter(id=nid).first()
    ...
    # 根据搜索条件去数据库中获取
    query_set = models.NC_VJ_ContestData.objects.filter(nC_VJ_Contest_id=nid, **data_dict)
    # 分页
    page_object = Pageination(request, query_set, page_size=20)
    context = {
        'query_set': page_object.page_query_set,
    ...
    return render(request, 'NC_VJ_contest_detail_list.html', context)
...
def select_contest_detail_bar(request):
    ...
    data_solved = []
    data_time = []
    for summary in sorted_team_summary_list:
...
def contest_detail_bar(request):
    """ 构造牛客比赛柱状图的数据 """
    info = models.NC_VJ_ContestData.objects.filter(nC_VJ_Contest_id=nid_x).all()
    legend = ["解题数"]
    data_name = []
    data_solved = []
    for obj in info:

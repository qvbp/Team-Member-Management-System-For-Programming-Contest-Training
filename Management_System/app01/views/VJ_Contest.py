...
def contest_list(request):
    ...
    # 根据搜索条件去数据库中获取
    query_set = models.VJudgeContest.objects.filter(**data_dict).order_by('-num')
    ...
    return render(request, 'VJ_contest_list.html', context)
...
def contest_add(request):
    ...
    exists = models.VJudgeContest.objects.filter(num=number).exists()
    if exists:
...
def contest_delete(request):
    ...
    exists = models.VJudgeContest.objects.filter(id=uid).exists()
    if not exists:
        return JsonResponse({"status": False, 'error': "删除失败，数据不存在。"})
    models.VJudgeContest.objects.filter(id=uid).delete()
    return JsonResponse({"status": True})
...
def select_contest_detail_list(request):
    if request.method == 'POST':
        ...
        # 处理 GET 请求，获取之前处理的数据
        # 注意：这里需要替换成你实际的查询逻辑
        team_summary = models.VJudgeContestData.objects.filter(vJudgeContest__id__in=contest_ids).values(
            'name').annotate(
            total_solved=Sum('solved'),
            total_penalty=Sum('time')
        )
        ...
        return render(request, 'VJ_select_contest_detail_list.html',
...
def contest_detail_list(request, nid):
    ...
    nid_x = nid
    # 获取比赛的名称
    contest = models.VJudgeContest.objects.filter(id=nid).first()
    ...
    # 根据搜索条件去数据库中获取
    query_set = models.VJudgeContestData.objects.filter(vJudgeContest_id=nid, **data_dict)
    # 分页
    page_object = Pageination(request, query_set, page_size=20)
    context = {
        'query_set': page_object.page_query_set,
    ...
    return render(request, 'VJ_contest_detail_list.html', context)
...
def contest_detail_bar(request):
    """ 构造牛客比赛柱状图的数据 """
    info = models.VJudgeContestData.objects.filter(vJudgeContest_id=nid_x).all()
    legend = ["解题数"]
    data_name = []
    data_solved = []
    for obj in info:
